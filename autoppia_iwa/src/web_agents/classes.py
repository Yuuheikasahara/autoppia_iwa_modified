...
def task_use_case_name_for_miner(task: Task) -> str | None:
    """Event name from task.use_case for miner payloads (POST /act, /solve_task_at_once)."""
    uc = getattr(task, "use_case", None)
    if uc is None:
        return None
