...
@pytest.mark.asyncio
async def test_solve_task_includes_use_case_in_json():
    agent = ApifiedOneShotWebAgent(base_url="http://localhost:9999")
    task = Task(
        url="https://example.com",
        prompt="Do thing",
        web_project_id="dummy",
        use_case=SimpleNamespace(name="BOOK_DETAIL"),
    )
    response_mock = AsyncMock()
    response_mock.raise_for_status = MagicMock()
    response_mock.json = AsyncMock(return_value={"actions": [], "web_agent_id": "m"})
    post_mock = MagicMock()
    ...
    assert posted[0][0] == "http://localhost:9999/solve_task_at_once"
    assert posted.kwargs["json"]["use_case"] == "BOOK_DETAIL"
