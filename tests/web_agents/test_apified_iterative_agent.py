...
from autoppia_iwa.src.web_agents.classes import task_use_case_name_for_miner
...
class TestTaskUseCaseName:
    def test_none_when_no_use_case(self):
        task = Task(url="https://example.com", prompt="x", web_project_id="p")
        assert task_use_case_name_for_miner(task) is None

    def test_from_string(self):
        task = Task(url="https://example.com", prompt="x", web_project_id="p", use_case="  LOGIN  ")
        assert task_use_case_name_for_miner(task) == "LOGIN"

    def test_from_object_name(self):
        task = Task(
            url="https://example.com",
            prompt="x",
            web_project_id="p",
            use_case=SimpleNamespace(name="FILM_DETAIL"),
        )
        assert task_use_case_name_for_miner(task) == "FILM_DETAIL"
...
class TestAct:
    ...
    async def test_act_returns_legacy_actions_on_success(self):
        ...
        assert len(result) == 1
        assert result[0].type == "ClickAction"
        posted = session_mock.post.call_args
        assert posted is not None
        assert posted.kwargs["json"].get("use_case") is None

    @pytest.mark.asyncio
    async def test_act_includes_use_case_in_json(self):
        agent = ApifiedWebAgent(base_url="http://localhost:9999", id="a1")
        task = Task(
            url="https://example.com",
            prompt="Click it",
            web_project_id="dummy",
            use_case=SimpleNamespace(name="SEARCH_FILM"),
        )
        response_mock = AsyncMock()
        response_mock.status = 200
        response_mock.raise_for_status = MagicMock()
        response_mock.json = AsyncMock(return_value={"actions": [{"type": "ClickAction", "x": 1, "y": 2}]})
        post_mock = MagicMock()
        post_mock.__aenter__ = AsyncMock(return_value=response_mock)
        post_mock.__aexit__ = AsyncMock(return_value=None)
        session_mock = MagicMock()
        ...
        posted = session_mock.post.call_args
        assert posted.kwargs["json"]["use_case"] == "SEARCH_FILM"
